...
public class SearchServlet extends HttpServlet {
	...
	void sendSearchResult(HttpServletResponse response) throws IOException {
		...
		try {
			...
			out.println("<TH></TH>");
			out.println("</TR>");
			String sql = "SELECT Id, nome, email" + " FROM exemplo1" + " WHERE Nome LIKE '%" + keyword + "%'"
			...
			while (rs.next()) {
				...
				out.println("<TD><A HREF=update?id=" + id + ">Atualizar</A></TD>");
				out.println("</TR>");
			}
			s.close();
			con.close();
	...
	/**
	 * Send the HTML page header, including the title and the <BODY> tag
	 */
	...
	/**
	 * Send the HTML page footer, i.e. the </BODY> and the </HTML>
	 */
